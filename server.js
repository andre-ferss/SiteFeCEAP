...
server.get("/sorvetin", function(req, res){
    return res.render("sorvetin.html");
})
...
server.post("/update", function(req, res){
    ...
    const query = `
    UPDATE projetos 
    SET img=?, title=?, category=?, description=?, url=? WHERE id=?`;
    const values = [req.body.img,req.body.title,req.body.category,req.body.description,req.body.url,req.body.id];
    bd.run(query, values, function(err){
        if(err) return console.log(err);        
